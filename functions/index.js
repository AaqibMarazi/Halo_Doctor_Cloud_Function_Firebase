...
exports.withdrawRequest = functions.firestore.document('/WithdrawRequest/{withdrawRequestId}').onCreate(async (snapshot, context) => {
    let userId = snapshot.data().userId;
    console.log('user Id : ' + userId);
    console.log('snapshot data : ' + JSON.stringify(snapshot.data()));
    let doctorId = await db.collection('Users').doc(userId).get().then((doc) => { return doc.data().doctorId });
    console.log('doctor id : ' + doctorId);
    //decrease doctor balance amount
    await db.collection('Doctors').doc(doctorId).get().then((querySnapshot) => {
...
exports.confirmConsultation = functions.firestore.document('/Order/{orderId}').onUpdate(async (change, context) => {
    const newValue = change.after.data();
    const previousValue = change.before.data();

    if (newValue.status == 'success' && previousValue.status == 'payment_success') {

        //get doctor timeslot
        let timeSlot = await db.collection('DoctorTimeslot').doc(previousValue.timeSlotId).get();

        //increase doctor balance by order ammount
        await db.collection('Doctors').doc(timeSlot.data().doctorId).get().then((querySnapshot) => {
            let doctorBalance = querySnapshot.data().balance;
        ...
        let userId = await db.collection('Users').where('doctorId', '==', timeSlot.data().doctorId).get().then(async (querySnapshot) => {
            let doctorId = {};
